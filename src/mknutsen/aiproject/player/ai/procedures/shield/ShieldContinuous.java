...
public class ShieldContinuous extends ShieldingArchitype {
    ...
    public void run() {
        while (isRoutineOn()) {
            for (Bullet bill : getPlayer().findEnemyBullets(getPlayer().getGame().getBullets())) {
                int playerOuter =
                        getPlayer().isPlayer1() ? getPlayer().getX() + getPlayer().getWidth() : getPlayer().getX();
                if (getPlayer().isOverlapping(
                        new RectangleGraphicObject(getPlayer().getX(), bill.getY(), Config.BULLET_WIDTH,
                                Config.BULLET_HEIGHT, false, null) {
                        ...
                        }) && Math.abs(bill.getX() - playerOuter) <
                        Config.BULLET_SPEED_PER_10_MS * Config.SHIELD_MULTIPLIER_FOR_SAFETY) {
                    getPlayer().shield();
                }
            }
            chill();
